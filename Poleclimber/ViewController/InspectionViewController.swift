...
//  Poleclimber
//
//  Created by Siva Preasad Reddy on 01/04/20.
...
class InspectionViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, NVActivityIndicatorViewable {
...
// MARK: - UIImagePickerControllerDelegate Method
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage, let data = image.jpegData(compressionQuality: 0.8), let mediaMetadata = info[UIImagePickerController.InfoKey.mediaType] as? String {
 ...
 @IBAction func uploadAction(_ sender: Any) {
    let parameters = [String: AnyObject]()
        ...
        showLoader()
        PWebService.sharedWebService.callWebAPIWith(httpMethod: "POST", apiName: "createclime", parameters: parameters) { (response, error) in
...
extension InspectionViewController: AXPhotosViewControllerDelegate {
